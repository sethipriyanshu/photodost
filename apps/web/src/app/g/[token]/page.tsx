...
import { getAlbumTeaser } from "@/lib/albums";
...
export default async function GuestPage({ params }: PageProps) {
  ...
  // Surfaced as a dismissible invite on arrival plus a header chip — an inline
  // block below the gallery is always off-screen on a phone.
  const teaser = await getAlbumTeaser(event.id);
  ...
  return (
    <GuestExperience
      ...
      album={teaser ? { href: `/g/${token}/album`, ...teaser } : null}
    />
  );
}
